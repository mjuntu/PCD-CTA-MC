%% Compute MC simulation. Note: this version is with bootstrapping using the Vecsey-Nagy diagnostic path. The other version estimates probabilities of misclassification from the patient data collection
...
func_prob.ECG = 0.101;
func_prob.ECHO = 0.221;
func_prob.SPECT = 0.678;
func_prob.ICA = 0.081;
        ...
        if isempty(idxs)
            ...
            switch CAD_RADS_SCORE_PRED
                ...
                case 3
                    ...
                    functional_option = randsample({'ECG','ECHO','SPECT'}, 1, true, [func_prob.ECG, func_prob.ECHO, func_prob.SPECT]);
                    additional_ICA = randsample({'NO','YES'},1,true,[1-func_prob.ICA,func_prob.ICA]);

                    % Assign index (1 = ECG, 2 = ECHO, 3 = SPECT)
                    idx = find(strcmp({'ECG', 'ECHO', 'SPECT'}, functional_option));
                    functional_cost = costs.(functional_option{1});
                    
                    % Store in local accumulators
                    total_cost_local(idx) = functional_cost;
                    test_count_local(idx) = 1;

                    % Check for additional ICA
                    if strcmp(additional_ICA, 'YES')
                        if CAD_RADS_SCORE_TRUE<4
                            ICA_cost = costs.ICA;
                        else
                            ICA_cost = costs.ICA_PCI;
                        end
                        total_cost_local(4) = ICA_cost; % Index 4 = ICA
                        test_count_local(4) = 1;
                    end

                    treatment_cost = costs.CTA + functional_cost + ICA_cost;
                ...
                case 4
                    ...
                    additional_ICA = randsample({'NO','YES'},1,true,[1-prob_ICA_CAD_RADS4,prob_ICA_CAD_RADS4]);
                    if strcmp(additional_ICA, 'YES')
                        if CAD_RADS_SCORE_TRUE<4
                            ICA_cost = costs.ICA;
                        else
...
CAD_RADS_SCORE_PREDS_Force = CAD_RADS_SCORE_PREDS;
CAD_RADS_SCORE_TRUES_Force = CAD_RADS_SCORE_TRUES;
